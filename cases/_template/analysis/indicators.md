# Indicators



Use this file to document indicators reviewed during the case.



Only document sensitive indicators if policy allows it. For public or portfolio use, sanitize or generalize indicators.



## Indicator Summary



**Case Title:**

**Date Reviewed:**

**Analyst:**

**Operating Mode:** Personal Lab / Company-Safe Template / Sanitized Export



## Indicator Table



| Indicator | Type | Source | Context | Status | Notes |

|---|---|---|---|---|---|

|  | IP Address |  |  | Known / Unknown / Suspicious / Benign |  |

|  | Domain |  |  | Known / Unknown / Suspicious / Benign |  |

|  | URL |  |  | Known / Unknown / Suspicious / Benign |  |

|  | Email Sender |  |  | Known / Unknown / Suspicious / Benign |  |

|  | File Hash |  |  | Known / Unknown / Suspicious / Benign |  |

|  | Hostname |  |  | Known / Unknown / Suspicious / Benign |  |

|  | User / Account |  |  | Known / Unknown / Suspicious / Benign |  |

|  | Process Name |  |  | Known / Unknown / Suspicious / Benign |  |

|  | Event ID |  |  | Known / Unknown / Suspicious / Benign |  |



## Known Good / Expected Indicators



List indicators that appear expected or benign.



-

-

-



## Suspicious Indicators



List indicators that may need more review.



-

-

-



## Unknown Indicators



List indicators where context is missing.



-

-

-



## Indicator Questions



Use these questions during triage:



- Is this indicator tied to the alert?

- Is this indicator known in the environment?

- Is this indicator expected for the user, host, or business process?

- Has this indicator appeared before?

- Is there successful activity tied to the indicator?

- Is the indicator associated with known malicious behavior?

- Does the indicator require escalation?

- Does the indicator require human-approved action?



## Safe Handling Notes



Do not publicly share:



- Real user names

- Email addresses

- Hostnames

- Internal IP addresses

- Customer identifiers

- Ticket numbers

- Internal domains

- Restricted company indicators



For sanitized reporting, use generalized wording.



Example:



> Reviewed repeated authentication activity from a source address against a user account.



## Recommended Next Steps



| Priority | Recommendation | Reason | Approval Needed? |

|---|---|---|---|

| High |  |  | Yes / No |

| Medium |  |  | Yes / No |

| Low |  |  | Yes / No |



## Closeout Review



Before closing this section, confirm:



- [ ] Indicators are documented clearly.

- [ ] Sensitive indicators are protected.

- [ ] Unknown indicators are marked.

- [ ] Suspicious indicators have recommended next steps.

- [ ] Any blocking or containment action is marked as requiring approval.

