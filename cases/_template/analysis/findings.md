\# Findings



Use this file to document the main findings from the investigation.



A finding should explain what happened, why it matters, what evidence supports it, and what should happen next.



\## Finding Summary



\*\*Case Title:\*\*  

\*\*Date Reviewed:\*\*  

\*\*Analyst:\*\*  

\*\*Operating Mode:\*\* Personal Lab / Company-Safe Template / Sanitized Export  

\*\*Finding Status:\*\* Draft / Reviewed / Escalated / Closed  



\## Finding Title



Write a clear title.



Example:



> Repeated Failed Authentication Activity Reviewed



\## Finding Type



Choose one or more:



\- \[ ] Authentication activity

\- \[ ] Phishing

\- \[ ] Malware

\- \[ ] Endpoint alert

\- \[ ] Network activity

\- \[ ] Suspicious command execution

\- \[ ] Account compromise

\- \[ ] Vulnerability

\- \[ ] Policy violation

\- \[ ] Detection gap

\- \[ ] Process gap

\- \[ ] Other



\## Severity



Choose one:



\- \[ ] Low

\- \[ ] Medium

\- \[ ] High

\- \[ ] Critical



\## Known Facts



List only confirmed facts.



\- 

\- 

\- 



\## Evidence Supporting the Finding



| Evidence | Source / Location | What It Shows |

|---|---|---|

|  |  |  |

|  |  |  |



Do not include restricted evidence unless policy allows it.



\## Analyst Interpretation



Explain what the evidence may indicate.



Do not overstate the conclusion.



\- 

\- 

\- 



\## Assumptions



List anything that is not confirmed.



\- 

\- 

\- 



\## Missing Information



List what would help confirm or rule out the finding.



\- 

\- 

\- 



\## Impact



Explain the possible impact.



Consider:



\- User/account impact

\- Host/device impact

\- Business process impact

\- Data exposure risk

\- Operational impact

\- Detection or visibility impact



\## Likelihood



Choose one:



\- \[ ] Low

\- \[ ] Medium

\- \[ ] High



Explain why:



> 



\## Risk Rating



Choose one:



\- \[ ] Low

\- \[ ] Medium

\- \[ ] High

\- \[ ] Critical



Explain why:



> 



\## Recommended Action



| Priority | Recommendation | Reason | Approval Needed? |

|---|---|---|---|

| High |  |  | Yes / No |

| Medium |  |  | Yes / No |

| Low |  |  | Yes / No |



\## Human Approval Required



Mark any action that requires approval.



\- \[ ] Block IP/domain

\- \[ ] Disable account

\- \[ ] Reset password

\- \[ ] Delete/quarantine file

\- \[ ] Isolate endpoint

\- \[ ] Contact user/customer

\- \[ ] Escalate to leadership

\- \[ ] Close incident

\- \[ ] Publish report

\- \[ ] Send external message



\## Detection / Control Notes



Use this section if the finding shows a detection, logging, or process gap.



\- Existing detection:

\- Gap identified:

\- Recommended improvement:

\- Owner:

\- Follow-up needed:



\## Framework Mapping



Optional mapping if supported by evidence.



\*\*MITRE ATT\&CK:\*\*  

\*\*NIST CSF:\*\*  

\*\*NIST SP 800-61:\*\*  

\*\*CIS Controls:\*\*  

\*\*Other:\*\*  



\## Final Finding Statement



Write a short final statement.



Example:



> Based on the available evidence, repeated failed authentication activity was reviewed. Compromise was not confirmed from the current evidence. Additional context may be needed before closure or escalation.



\## Closeout Review



Before closing this finding, confirm:



\- \[ ] Facts are separated from assumptions.

\- \[ ] Evidence supports the finding.

\- \[ ] Severity is reasonable.

\- \[ ] Impact is explained.

\- \[ ] Risk is not overstated.

\- \[ ] Recommended actions are safe and actionable.

\- \[ ] Approval-required actions are marked.

\- \[ ] Sensitive information is protected.

