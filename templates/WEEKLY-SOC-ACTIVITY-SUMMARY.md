\# Weekly SOC Activity Summary



\## Report Details



\*\*Week Covered:\*\*  

\*\*Prepared By:\*\*  

\*\*Team / Queue:\*\*  

\*\*Date Prepared:\*\*  

\*\*Status:\*\* Draft / Reviewed / Final  



\## Executive Summary



Briefly summarize the week in plain English.



Include:



\- Overall alert volume

\- Major trends

\- Notable investigations

\- Key risks

\- Improvements made

\- Items needing follow-up



Example:



> This week’s SOC activity was mostly routine, with failed login activity and phishing reviews making up the largest portion of analyst work. No confirmed compromise was identified based on the available evidence, but several alerts required follow-up due to repeated authentication failures and incomplete asset context.



\## Weekly Metrics



| Metric | Count | Notes |

|---|---:|---|

| Total alerts reviewed |  |  |

| Alerts closed as benign |  |  |

| Alerts escalated |  |  |

| Confirmed incidents |  |  |

| Phishing emails reviewed |  |  |

| Endpoint alerts reviewed |  |  |

| Vulnerability items reviewed |  |  |

| Detection tuning opportunities |  |  |

| Open follow-up items |  |  |



\## Alert Categories



| Category | Volume | Trend | Notes |

|---|---:|---|---|

| Authentication |  | Up / Down / Stable |  |

| Phishing |  | Up / Down / Stable |  |

| Endpoint |  | Up / Down / Stable |  |

| Network |  | Up / Down / Stable |  |

| Malware |  | Up / Down / Stable |  |

| Vulnerability |  | Up / Down / Stable |  |

| Other |  | Up / Down / Stable |  |



\## Notable Cases



| Case | Severity | Status | Summary | Next Step |

|---|---|---|---|---|

|  |  |  |  |  |



\## Known Facts



List confirmed observations from the week.



\- 

\- 

\- 



\## Analyst Interpretation



Explain what the activity may suggest.



Do not overstate the conclusion.



\- 

\- 

\- 



\## Assumptions



List anything that is not confirmed.



\- 

\- 

\- 



\## Missing Information



List gaps that limited analysis.



Examples:



\- Asset owner unknown

\- User context unavailable

\- Logs not retained long enough

\- Alert lacked process details

\- No baseline available

\- Ticket did not include business context



\## Detection and Tuning Notes



Document alerts that may need tuning, suppression review, enrichment, or detection improvement.



| Detection / Alert | Issue | Recommendation | Approval Needed? |

|---|---|---|---|

|  |  |  | Yes / No |



\## Response Actions Completed



| Action | Case / Alert | Completed By | Notes |

|---|---|---|---|

|  |  |  |  |



\## Human Approval Items



List any actions that require approval before execution.



| Requested Action | Reason | Risk | Approval Owner |

|---|---|---|---|

|  |  |  |  |



Examples of approval-required actions:



\- Blocking IPs or domains

\- Disabling accounts

\- Resetting passwords

\- Isolating endpoints

\- Deleting or quarantining files

\- Contacting users or customers

\- Closing incidents with business impact



\## Risks and Watch Items



\- 

\- 

\- 



\## Lessons Learned



Summarize what the team learned this week.



\- 

\- 

\- 



\## Recommended Next Steps



| Priority | Recommendation | Reason | Owner |

|---|---|---|---|

| High |  |  |  |

| Medium |  |  |  |

| Low |  |  |  |



\## Sanitization Reminder



Before sharing this report outside the approved audience, remove:



\- Usernames

\- Emails

\- Hostnames

\- IP addresses

\- Ticket IDs

\- Customer information

\- Internal tool screenshots

\- Company-sensitive process details



\## Final Review Checklist



Before marking final:



\- Metrics are accurate.

\- Known facts are separated from assumptions.

\- Recommendations are actionable.

\- Approval-required items are marked.

\- Sensitive details are protected.

\- The summary is readable by a non-technical manager.

\- The report does not claim actions were taken unless confirmed.

